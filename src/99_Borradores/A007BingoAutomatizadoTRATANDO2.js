...
export const A007BingoAutomatizado = () => {
    ...
    const [ CARTONESCONBINGO, setCARTONESCONBINGO ] = useState([])
    ...
    const [ formasDHB, DISPFormasDHB] = useReducer((stateFormasDeHB = initialFormasDeHacerBingo, actionFORMASDHB) => {       
        ...
        switch (actionFORMASDHB.type) {  
          ...
          case typesFormasDHB.typesFormasDHB4ESQUINAS : {  
            ...
            /* let cargarCartonAArray = {
                cartonID: actionFORMASDHB.payload.cartonID, 
                formaDeHacerBingo: actionFORMASDHB.payload.tipoDBingo, 
                src: "4Esquinas", 
                PorcentajeDePago: 0.1, 
                Bingo: true
            } */
            
            (CARTONESCONBINGO.length !== 0)
            ?  CARTONESCONBINGO.map((cartones, index) => {
                    ...
                    // console.log("Valor de cartones.cartonID = ", cartones.cartonID);
                    // console.log("Valor de CartonIDConBingo = ", CartonIDConBingo);
                    // console.log("Valor de cartones.formaDeHacerBingo = ", cartones.formaDeHacerBingo);
                    // console.log("Bingo4Esquinas = ", Bingo4Esquinas);
                    if (cartones.cartonID === CartonIDConBingo && cartones.formaDeHacerBingo === Bingo4Esquinas) {
                        console.log("TRUE"); 
                        /* setCARTONESCONBINGO([
                            ...CARTONESCONBINGO, {
                                cartonID: actionFORMASDHB.payload.cartonID, 
            ...
            /* (CARTONESCONBINGO.length !== 0)
                ?   //   Object.entries(CARTONESCONBINGO).forEach(([key, value]) => {
                    //         console.log(value)
                    //     });
                    CARTONESCONBINGO.map(cartones => (
                        (cartones.cartonID === CartonIDConBingo) && (cartones.formaDeHacerBingo === Bingo4Esquinas)
                          ?  console.log("SI EXISTE EL CARTON CON ESTA FORMA DE HACER BINGO = ", cartones)
                          :  setCARTONESCONBINGO([
                            ...CARTONESCONBINGO,
        ...
        for (var i = 0; i < arrayMatrixBINGO[0].length; i++) {
            ...
            if(candado === false){  
                ...
                if(celda01.FondoRed === true &&  celda05.FondoRed === true && celda21.FondoRed === true && celda25.FondoRed === true){
                    let CartConBingo4Esq = {
                        cartonID: "CARTON-"+[i+1], 
                        formaDeHacerBingo: "4 ESQUINAS", 
                        src: "4Esquinas", 
                        PorcentajeDePago: 0.1, 
                        CartonYBingo: "CARTON-"+[i+1]+" "+"4 ESQUINAS",
                    };
                    if(CARTONESCONBINGO && CARTONESCONBINGO.CartonYBingo === CartConBingo4Esq.CartonYBingo){
                        console.log("CARTON YA EN LA MEMORIA DE LA VARIABLE");
                    }else{
                        setCARTONESCONBINGO([
                            ...CARTONESCONBINGO, CartConBingo4Esq
                        ]) 
                    }
